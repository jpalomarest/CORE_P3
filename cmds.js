...
exports.playCmd = rl => {
	...
	const playQuiz = () => {
		...
		if(idFalta.length > 0){
			let pos = Math.floor(Math.random()*idFalta.length);
			let id = idFalta[pos];
			const juego = model.getByIndex(id);
			idFalta.splice(pos,1);

			rl.question(colorize('¿'+juego.question+'?     ', 'red'), answer => {
            
            if(answer.toLowerCase().trim() === juego.answer.toLowerCase()){
                score++;
